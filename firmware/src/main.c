...
void Move_Motor(uint8_t AmplNode,double ToGo,bool IsRelative)
{
...
double Req_AZ_Position,Req_EL_Position;
    ...
    switch(*(ProtoPtr->BufferFor_DataRcv+2))
    {
        case 0x1: //position //3,4 AZ (359.99)  5,6(+-90) EL position
            ...
            Move_Motor(AZ_Amplifier,Req_AZ_Position,true);
            break;
        case 0x08: //AZ INCH 
 ...
 double Angle;
  int32_t Position;
    ...
    if(!Init_Amplifier(AZ_Amplifier,Ampl_POSITION_Mode))
    {
        ...
        LongBeep();
        //Get_and_Display_Ampl_Error(AmplNode);
        //return;
    }
    //else
...
//                        
                        ...
                        break;
                   case ELKEY:
                        Move_Motor(AZ_Amplifier,10.0,true); //relative
                        break;
                   case AZKEY:
                        //Move_Motor(AZ_Amplifier,-90.0,false); //absolute
                        ...
                        if(Enter_float_Data(1,6,2,14,&Angle,30))
                        {
                            ...
                            ShortBeep();
                                ...
                                Move_Motor(AZ_Amplifier,Angle,false); //absolute
                        }
                        else
                        {
                            ClearDisp(1);
